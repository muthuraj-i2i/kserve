...
var _ = Describe("v1beta1 inference service controller", func() {
	...
	Context("When creating inference service with raw kube predictor", func() {
		type testCase struct {
			name                  string
			serviceName           string
			autoscalerClass       string
			deploymentStrategy    *appsv1.DeploymentStrategy
			minReplicas           *int32
			maxReplicas           int32
			timeoutSeconds        *int64
			autoScalingMetrics    []v1beta1.MetricsSpec
			expectHPA             bool
			expectedHPAMetrics    []autoscalingv2.MetricSpec
			expectedTimeout       string
			testMinReplicasUpdate bool
		}

		testCases := []testCase{
			{
				name:            "Should have httproute/service/deployment/httproute created",
				serviceName:     "raw-foo",
				autoscalerClass: string(constants.AutoscalerClassHPA),
			...
			{
				name:               "Should have httproute/service/deployment/hpa created with DeploymentStrategy",
				...
				autoScalingMetrics: []v1beta1.MetricsSpec{
					{
						...
						Resource: &v1beta1.ResourceMetricSource{
							...
							Target: v1beta1.MetricTarget{
								...
								AverageUtilization: func() *int32 { val := int32(75); return &val }(),
							},
						},
					},
				},
			...
			{
				name:                  "Should have httproute/service/deployment created",
				serviceName:           "raw-foo-2",
				autoscalerClass:       string(constants.AutoscalerClassNone),
				expectHPA:             false,
				expectedTimeout:       "60s",
				testMinReplicasUpdate: true,
			},
		}

		DescribeTable("InferenceService creation tests",
			func(tc testCase) {
				...
				expectedRequest := reconcile.Request{NamespacedName: types.NamespacedName{Name: tc.serviceName, Namespace: "default"}}
				...
				isvc := &v1beta1.InferenceService{
					ObjectMeta: metav1.ObjectMeta{
						...
						Annotations: map[string]string{
							...
							constants.AutoscalerClass: tc.autoscalerClass,
						},
					},
					Spec: v1beta1.InferenceServiceSpec{
						Predictor: v1beta1.PredictorSpec{
							ComponentExtensionSpec: v1beta1.ComponentExtensionSpec{
								...
								DeploymentStrategy: tc.deploymentStrategy,
								AutoScaling: func() *v1beta1.AutoScalingSpec {
									if tc.autoScalingMetrics != nil {
										return &v1beta1.AutoScalingSpec{Metrics: tc.autoScalingMetrics}
									}
									return nil
								}(),
							},
				...
				// Build expected deployment spec
				...
				if tc.deploymentStrategy != nil {
					expectedDeploymentStrategy = *tc.deploymentStrategy
				}
				...
				expectedDeployment := &appsv1.Deployment{
					...
					Spec: appsv1.DeploymentSpec{
						...
						Template: corev1.PodTemplateSpec{
							ObjectMeta: metav1.ObjectMeta{
								...
								Labels: map[string]string{
									...
									constants.InferenceServicePodLabelKey: tc.serviceName,
								},
								Annotations: map[string]string{
						...
						Strategy:                expectedDeploymentStrategy,
				...
				expectedService := &corev1.Service{
					...
					Spec: corev1.ServiceSpec{
						Ports: []corev1.ServicePort{
							{
								Name:       constants.PredictorServiceName(tc.serviceName),
								Protocol:   "TCP",
								Port:       80,
						...
						Selector: map[string]string{
							"app": "isvc." + constants.PredictorServiceName(tc.serviceName),
						},
					},
				}
						...
						Rules: []gwapiv1.HTTPRouteRule{
							{
								...
								Timeouts: &gwapiv1.HTTPRouteTimeouts{
									Request: ptr.To(gwapiv1.Duration(tc.expectedTimeout)),
								},
							},
						},
				...
				expectedIsvcStatus := v1beta1.InferenceServiceStatus{
					...
					URL: &apis.URL{
						Scheme: "http",
						Host:   fmt.Sprintf("%s-default.example.com", serviceKey.Name),
					},
					...
					Components: map[v1beta1.ComponentType]v1beta1.ComponentStatusSpec{
						v1beta1.PredictorComponent: {
							...
							URL: &apis.URL{
								Scheme: "http",
								Host:   fmt.Sprintf("%s-predictor-default.example.com", serviceKey.Name),
							},
						},
					},
				...
				// Add DeploymentMode for non-autoscalerClassNone tests
				if tc.autoscalerClass != string(constants.AutoscalerClassNone) {
					expectedIsvcStatus.DeploymentMode = string(constants.RawDeployment)
				}
				...
				// Test minReplicas update for AutoscalerClassNone case
				if tc.testMinReplicasUpdate {
			...
			Entry("basic HPA test", testCases[0]),
			Entry("custom deployment strategy test", testCases[1]),
			Entry("autoscaler class none test", testCases[2]),
		)
	})
	...
	Context("When creating inference service with raw kube predictor and `serving.kserve.io/stop`", func() {
		...
		Describe("inference service only", func() {
			// Define test case struct for table-driven tests
			type testCase struct {
				name                string
				serviceName         string
				...
				useOTel             bool
				initialState        bool   // true = start with annotation set, false = start without annotation
				updatedAnnotation   *string // nil = no update, otherwise the new value
				expectResources     bool   // whether resources should be created/exist
				expectIngressNotGW  bool   // true = expect Ingress not Gateway API
				expectKeda          bool   // true = expect Keda, false = expect HPA
				expectOTel          bool   // true = expect OpenTelemetry
			}
			
			// Define test cases for table-driven tests
			testCases := []testCase{
				{
					name:              "keep resources with annotation false",
					...
					expectIngressNotGW: false,
					expectKeda:        false,
					expectOTel:        false,
				},
				{
					name:              "keep resources with annotation false and gateway API disabled",
					serviceName:       "stop-false-ingress-isvc",
					stopAnnotation:    "false",
					autoscalerClass:   string(constants.AutoscalerClassKeda),
					gatewayAPIEnabled: false,
					useOTel:           true,
					initialState:      false,
					updatedAnnotation: nil,
					expectResources:   true,
					expectIngressNotGW: true,
					expectKeda:        true,
					expectOTel:        true,
				},
				{
					name:              "not create resources with annotation true",
				...
				{
					name:              "not create resources with annotation true and gateway API disabled",
				...
				{
					name:              "delete resources when annotation updated to true",
					...
					useOTel:           false,
					initialState:      false,
					updatedAnnotation: proto.String("true"),
					expectResources:   false,
					expectIngressNotGW: false,
					expectKeda:        false,
					expectOTel:        false,
				...
				{
					name:              "delete resources when annotation updated to true with gateway API disabled",
				...
				{
					name:              "create resources when annotation updated to false",
					...
					useOTel:           false,
					initialState:      false,
					updatedAnnotation: proto.String("false"),
					expectResources:   true,
					expectIngressNotGW: false,
					expectKeda:        false,
					expectOTel:        false,
				...
				{
					name:              "create resources when annotation updated to false with gateway API disabled",
			...
			DescribeTable("When converting to table driven test",
				func(tc testCase) {
				...
				expectedRequest := reconcile.Request{NamespacedName: types.NamespacedName{Name: tc.serviceName, Namespace: serviceNamespace}}
				serviceKey := expectedRequest.NamespacedName
				storageUri := "s3://test/mnist/export"
				...
				isvc := defaultIsvc(serviceKey, storageUri, tc.autoscalerClass, qty)
				isvc.Annotations[constants.StopAnnotationKey] = tc.stopAnnotation
				...
				// Apply annotation update if specified
				if tc.updatedAnnotation != nil {
					updatedIsvc := expectIsvcToExist(ctx, serviceKey)
					stoppedIsvc := updatedIsvc.DeepCopy()
					stoppedIsvc.Annotations[constants.StopAnnotationKey] = *tc.updatedAnnotation
				...
				if tc.expectResources {
					...
					if tc.expectKeda {
						...
						expectResourceDoesNotExist(context.Background(), &autoscalingv2.HorizontalPodAutoscaler{}, predictorKey)
					}
			...
			Entry("keep resources with annotation false", testCases[0]),
			Entry("keep resources with annotation false and gateway API disabled", testCases[1]),
			Entry("not create resources with annotation true", testCases[2]),
			Entry("not create resources with annotation true and gateway API disabled", testCases[3]),
			Entry("delete resources when annotation updated to true", testCases[4]),
			Entry("delete resources when annotation updated to true with gateway API disabled", testCases[5]),
			Entry("create resources when annotation updated to false", testCases[6]),
			Entry("create resources when annotation updated to false with gateway API disabled", testCases[7]),
		)
		})
